...
std::vector<double> SmartFit(TH1F *h, TString title, double peakPos, double lowOff, double highOff, double rms, int padIdx, TCanvas *c, bool useExpo = false, bool useLangau = false, bool useAsym = false)
{
  c->cd(padIdx); 
  h->SetTitle(title);
  ...
  if (useAsym) sigOnly->SetParameter(5, f_fit->GetParameter(5));
  ...
  line_mean->SetLineStyle(2); line_mean->SetLineColor(kBlack); line_mean->Draw();
...
void regeneration_cross_section(TString root_file_date)
{
  ...
  for (auto &g : groups)
  {
    ...
    // 1. Fit Danych
    auto resData = SmartFit(g.data, g.name + " (DATA)", g.pos, g.lowOff, g.highOff, rms, 1, c1, false, false, false);
    TF1* fData = (TF1*)g.data->GetFunction("f_fit");

    // 2. Fit MC
    auto resMC = SmartFit(g.mc, g.name + " (MC SUM)", g.pos, g.lowOff, g.highOff, rms, 2, c1, false, false, false);
    TF1* fMC = (TF1*)g.mc->GetFunction("f_fit");

    // 3. Wyznaczanie analitycznej funkcji poprawkowej z pasmem błędów
    c1->cd(3);
    gPad->Clear();
    
    // Definicja ilorazu Gaussów: [0,1,2] - Dane, [3,4,5] - MC
    TF1 *f_weight = new TF1("f_weight", "([0]*exp(-0.5*((x-[1])/[2])^2)) / ([3]*exp(-0.5*((x-[4])/[5])^2))", resMC[0], resMC[1]);
    ...
    delete f_weight; delete gr_band;
  }
